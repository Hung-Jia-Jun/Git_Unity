...
public class NPCFeedBackOnTrigger : MonoBehaviour {
	...
	void Start () {
		...
		TextBox[30] = GameObject.Find("BackButton");
		TextBox[31] = GameObject.Find("Recognize");
		TextBox[32] = GameObject.Find("NextButton");
		...
		case 2:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"請問還有空房嗎");//傳入語音辨識驗證使用者的話
				}
		...
		case 4:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"我要一間單人房");//傳入語音辨識驗證使用者的話
				}
				//ButtonUI(/*ShowNext*/false,/*ShowBack*/true,/*ShowExit*/false);//玩家可返回上一步
		...
		case 6:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"我住一晚明早退房");//傳入語音辨識驗證使用者的話
				}
		...
		case 9:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"我要一間大街風景房");//傳入語音辨識驗證使用者的話(我要一間大街風景房)
				}
		...
		case 11:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"請問有附早餐嗎");//傳入語音辨識驗證使用者的話
				}
				//ButtonUI(/*ShowNext*/false,/*ShowBack*/true,/*ShowExit*/false);//UIButton顯示設定
		...
		case 13:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"好的請幫我辦理入住手續");//傳入語音辨識驗證使用者的話
				}
				//ButtonUI(/*ShowNext*/false,/*ShowBack*/true,/*ShowExit*/false);//UIButton顯示設定
		...
		case 15:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"我姓林名彼得");//傳入語音辨識驗證使用者的話
				}
				//ButtonUI(/*ShowNext*/false,/*ShowBack*/true,/*ShowExit*/false);//UIButton顯示設定
		...
		case 17:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"我這樣填寫有要更改的地方嗎");//傳入語音辨識驗證使用者的話
				}
				//ButtonUI(/*ShowNext*/false,/*ShowBack*/true,/*ShowExit*/false);//UIButton顯示設定
		...
		case 19:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"我是德國人所以有些特別");//傳入語音辨識驗證使用者的話
				}
				//ButtonUI(/*ShowNext*/false,/*ShowBack*/true,/*ShowExit*/false);//UIButton顯示設定
		...
		case 21:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"我的護照號碼是767741233");//傳入語音辨識驗證使用者的話
				}
				//ButtonUI(/*ShowNext*/false,/*ShowBack*/true,/*ShowExit*/false);//UIButton顯示設定
		...
		case 23:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"請問能讓我結帳嗎");//傳入語音辨識驗證使用者的話
				}
				//ButtonUI(/*ShowNext*/false,/*ShowBack*/true,/*ShowExit*/false);//UIButton顯示設定
		...
		case 25:
			{
				...
				if (Timer > 10)
				{
					SwitchRecognizeOk(/*語音辨識內容(限定前五個字)*/"我要付現金請你點一下");//傳入語音辨識驗證使用者的話
				}
				//ButtonUI(/*ShowNext*/false,/*ShowBack*/true,/*ShowExit*/false);//UIButton顯示設定
	...
	public void Recognize()//語音辨識
	{

		Input.WriteFile(/*要寫入的訊息*/"1",/*檔案路徑*/"C:\\A.txt",/*寫入模式true為加入行尾，false為清空後寫入，*/false);
		//CloseAllTextBox();//每次畫面刷新都先關閉所有物件
	...
	public void Call_Customer_Talk(/*陣列指標位置*/int i ,/*尋找陣列物件位置*/string ObjName,/*顧客要說的話*/string CustomerText)//顧客要說的話
	{
		TextBox[i] = GameObject.Find(ObjName);
